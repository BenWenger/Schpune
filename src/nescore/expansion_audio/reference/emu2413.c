/*
 * Schpune - Portable Nintendo Entertainment System Emulator
 * Copyright (C) 2008  Disch
 * 
...
static unsigned char default_inst[16 * 8]=
{
...
void OPLL_getDefaultPatch(e_int32 num, OPLL_PATCH *patch)
{
  OPLL_dump2patch(default_inst+num*8,patch) ;
}
...
static void makeDefaultPatch()
{
  e_int32 j ;
    ...
    for(j=0;j<16;j++)
      OPLL_getDefaultPatch(j,&default_patch[j*2]) ;
...
OPLL *OPLL_new(e_uint32 clk, e_uint32 rate)
{
  ...
  for( i = 0 ; i < 6 ; i++ )
  {
    ...
    if(ch[i]==NULL)
    {
      ...
      for ( j = 0 ; j < 16*2 ; j++ ) free(patch[j]) ;
      free(opll) ;
      return NULL ;
    }
  }
  
  for ( i = 0 ; i < 16*2 ; i++ ) opll->patch[i] = patch[i] ;
...
void OPLL_set_rate(OPLL *opll, e_uint32 c, e_uint32 r)
{
  maketables(c,r);
}
...
static void calc(OPLL *opll, e_uint8* mix)
{
  e_int32 i ;
  ...
  for(i = 0 ; i < 6 ; i++)
  {
    opll->prev[i] = opll->next[i];
    if(!mix[i] || opll->CAR(i)->eg_mode==FINISH)
      opll->next[i] = 0;
	else
      opll->next[i] = calc_slot_car(opll->CAR(i),calc_slot_mod(opll->MOD(i))) ;
  }
}

void OPLL_update(OPLL* opll, e_int32* transl, e_int32* transr, int len)
{
	int i, j;
	e_int16 out;
	...
	if(transr)
	{
		for(j = 0; j < len; j++)
		{
			while(opll->realstep > opll->oplltime)
			{
				...
				calc(opll,opll->sch_mix);
			}
			opll->oplltime -= opll->realstep ;

			for(i = 0; i < 6; i++)
			{
				out = (e_int16)(((double)opll->next[i] * (opll->opllstep - opll->oplltime)
				+(double)opll->prev[i] * opll->oplltime)/opll->opllstep);

				transl[j] += (e_int16)((out - opll->sch_transprev[i]) * opll->sch_multl[i]);
				if(opll->sch_inv[i])
					transr[j] -= (e_int16)((out - opll->sch_transprev[i]) * opll->sch_multr[i]);
				else
					transr[j] += (e_int16)((out - opll->sch_transprev[i]) * opll->sch_multr[i]);
				opll->sch_transprev[i] = out;
			}
		}
	}
	else
